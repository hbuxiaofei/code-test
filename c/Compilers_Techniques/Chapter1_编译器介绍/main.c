...
void List_reverse_print (struct List_t *list)
{
  ...
  while (node) {
    struct Stack_Push *p = (struct Stack_Push *)node->instr;
    if (p->kind == STACK_PUSH) {
      printf("%d", p->i);
    } else if (p->kind == STACK_ADD) {
      printf("+");
    }
    node = node->next;
  }
...
int main()
{
  ...
  struct Exp_t *exp = Exp_Sum_new(Exp_Sum_new(Exp_Int_new(2), Exp_Int_new(3)),
                                Exp_Int_new(4));
