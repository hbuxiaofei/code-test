...
# 0x1000:0000 system
# 这段代码没有需要进行修改的地方
# 有兴趣的可以仔细读一下 read_it 子程序

	.code16				 # 指定语法为十六位汇编
	.equ SYSSIZE, 0x3000 # System Size in Clicks(1 click = 16bits) 192k
...
_start:
	# 这里我们将启动扇区从0x07c0:0000处移动到0x9000:0000
	# rep mov 的用法如下
	mov $BOOTSEG, %ax			# 源地址 ds:si = 0x07c0:0000
	mov %ax, %ds
	xor %si, %si
	mov $INITSEG, %ax			# 目的地址 es:di = 0x9000:0000
	mov %ax, %es
	xor %di, %di
	mov $256, %cx				# 移动次数 %cx = 256
								# 因为是movsw所以每次移动一个Word(2Byte) 256 * 2 = 512 Byte	即为启动扇区的大小
	rep movsw					# 进行移动
								# 重复执行串 REP, REP (CX)=重复次数
								# MOVSB/MOVSW (move string byte/word)
	ljmp $INITSEG, $go			# 长跳转同时切换CS:IP

go:
	mov %cs, %ax			# CS的全拼为"Code Segment"，翻译为"代码段寄存器"
							# 对DS(数据段), ES(附加段), SS(堆栈段)寄存器进行初始化
	mov %ax, %ds
...
load_setup:
	...
	jnc setup_load_ok		# 没有异常，加载成功
							# int 13h中断传回cf=1表示错误, cf=0表示成功, 所以jnc就是成功
							# jnc不进位则跳转; jc进位则跳转
	mov $0x0000, %dx
...
read_it:
	mov %es, %ax
	test $0x0fff, %ax   # 0x0fff & ax(0x1000) = 0, zf = 0
die:
	jne die				# If es is not at 64KB(0x1000) Boundary, then stop here
						# jne = jnz, jne和jnz是同一条指令从两个不同角度给的命名, 编译后一是同一个机器码
	xor %bx, %bx
rp_read:
	mov %es, %ax
	cmp $ENDSEG, %ax
	jb ok1_read			# If %ES < $ENDSEG, then continue reading, else just return
	ret
ok1_read:
...
msg1:
	...
	.= 508                      # 这里是对齐语法 等价于 .org 表示在该处补零，一直补到 地址为 508 的地方
								# 然后在这里填充好0xaa55魔术值，BIOS会识别硬盘中第一扇区以0xaa55结尾的为启动扇区，
								# 于是BIOS会装载代码并且运行
