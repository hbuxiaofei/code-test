...
const (
	NET_COLUMN_TEXT = iota
)

// Append a toplevel row to the tree store for the tree view
func netAddRow(treeStore *gtk.TreeStore, text string) *gtk.TreeIter {
	return netAddSubRow(treeStore, nil, text)
}

// Append a sub row to the tree store for the tree view
func netAddSubRow(treeStore *gtk.TreeStore, iter *gtk.TreeIter, text string) *gtk.TreeIter {
	// Get an iterator for a new row at the end of the list store
	i := treeStore.Append(iter)

	err := treeStore.SetValue(i, NET_COLUMN_TEXT, text)
	if err != nil {
...
func netCreateTextColumn(title string, id int) *gtk.TreeViewColumn {
...
func netSelectionChangedHandler(st *gtk.TreeSelection, ts *gtk.TreeStore) {
...
func LoadNet(builder *gtk.Builder) error {

	// getting treeview
	obj, err := builder.GetObject("tv_net")
	util.ErrorCheck(err)
	computeTreeView, err := util.IsTreeView(obj)
	util.ErrorCheck(err)
	computeTreeView.AppendColumn(netCreateTextColumn("NetWare", NET_COLUMN_TEXT))
	...
	iterDatacenter := netAddRow(computeTreeStore, "bridge")
	iterCluster := netAddSubRow(computeTreeStore, iterDatacenter, "vswitch")
	iterHOst := netAddSubRow(computeTreeStore, iterCluster, "vnet")
	netAddSubRow(computeTreeStore, iterHOst, "vswitch0")
	netAddSubRow(computeTreeStore, iterHOst, "vnet0")
	netAddSubRow(computeTreeStore, iterHOst, "vnet1")
	...
	computeSelection.Connect("changed", netSelectionChangedHandler,
		computeTreeStore)
